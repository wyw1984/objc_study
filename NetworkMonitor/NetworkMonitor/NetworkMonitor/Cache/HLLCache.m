...
@property (nonatomic, strong)HLLDataEntiy *dao;

@end


@implementation HLLCache




+ (instancetype)sharedHLLCache {
    static HLLCache *cache = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!cache) {
            cache = [[HLLCache alloc] init];
        }
    });
    return cache;
}
...
- (void)cacheValue:(NSString *)value key:(NSString *)key traceId:(NSString *)traceId {
    ...
    @synchronized (self) {
        HLLDataModel *model = self.cache[traceId];
        if (!model) {
            model = [[HLLDataModel alloc] init];
            model.ti = traceId;
            model.ns = [HLLNetUtils getNetWorkInfo];
            model.apn = [HLLNetUtils getDetailApCode];
            model.signal = [HLLNetUtils getNetworkSignal];
            model.airplane = [HLLNetUtils getPhoneNetMode];
            model.dBm = [HLLNetUtils getDBMSignalStrength];
            model.ope = [HLLNetUtils getOperator];
...
//    }
    ...
    if ([HLLNetworkMonitorManager sharedNetworkMonitorManager].outputBlock) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [HLLNetworkMonitorManager sharedNetworkMonitorManager].outputBlock(traceId, [model toDictionary]);
